...
function Input_Pass_field({
  ...
  return (
    ...
    <div className="relative">
     ...
     <div className={`${showpassword?"hidden " :" "} absolute inset-y-0  pr-3 right-0 flex items-center`}  onClick={()=>setshowpassword(!showpassword)} ><i class={`fa-solid  ${showpassword?" fa-eye" :" fa-eye "}`}></i></div>
     <div className={`${showpassword?"" :"hidden "} absolute inset-y-0  pr-3 right-0 flex items-center`}  onClick={()=>setshowpassword(!showpassword)}><i class={"fa-solid fa-eye-slash"}></i></div>
     </div>
    </div>
    </>
  )
